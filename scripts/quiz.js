...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("quiz.js: DOMContentLoaded - selectedQuiz =", selectedQuiz);
...
function calculateScore() {
    ...
    console.log("quiz.js - calculateScore: currentUser =", currentUser);
    let users = JSON.parse(localStorage.getItem("users")) || [];
    console.log("quiz.js - calculateScore: users before update =", users);
    let selectedQuiz = localStorage.getItem("selectedQuiz");
    console.log("quiz.js - calculateScore: selectedQuiz =", selectedQuiz);

    const userIndex = users.findIndex(user => user.username === currentUser);
    console.log("quiz.js - calculateScore: userIndex =", userIndex);

    if (userIndex !== -1) {
        if (!users[userIndex].scores) {
            users[userIndex].scores = {};
        }
        users[userIndex].scores[selectedQuiz] = score;
        console.log("quiz.js - calculateScore: users after update =", users);
        localStorage.setItem("users", JSON.stringify(users));
        console.log("quiz.js - calculateScore: localStorage users after set =", JSON.parse(localStorage.getItem("users")));
    } else {
        console.log("quiz.js - calculateScore: User not found in users array");
    }
